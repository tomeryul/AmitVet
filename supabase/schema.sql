...
-- ---------- 7. תכונות נוספות (v2): משימות, מרשמים, משקל, מרפאה ----------
...
create table if not exists public.prescriptions (
  ...
  pet_id bigint not null references public.pets(id) on delete cascade,
  vet_id uuid references public.profiles(id) on delete set null,
  medication text not null, dosage text, instructions text,
...
create table if not exists public.weight_logs (
  id bigint generated always as identity primary key,
  pet_id bigint not null references public.pets(id) on delete cascade,
  weight_kg numeric not null, measured_at date not null default current_date, notes text,
  created_at timestamptz not null default now()
);

create table if not exists public.clinic_settings (
  id int primary key default 1 check (id = 1),
  clinic_name text, phone text, address text, hours text, emergency_info text,
