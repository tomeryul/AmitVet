...
## עיצוב — מותג AmitVet (mobile-first, דו-לשוני)

הממשק (ב-`docs/`) מעוצב לפי מותג **AmitVet** ("וטרינר עד הבית"): **קנבס קרם חמים**,
לוגו-מילה **Amit** (נייבי) + **Vet** (טורקיז) עם גג-בית וכלב, ואקסנט **ירוק-זית**
עם מוטיב ענפי-זית. מבנה **mobile-first**: app bar שקוף, **טאב-בר תחתון**, כפתורי
**FAB** (ירוקים), ו-**bottom-sheets** לטפסים. גופני **Rubik + Assistant** (עברית
*ואנגלית*), אייקוני **Lucide** דקים וממורכזים, ואווטארים בגרדיאנט — ללא אמוג'י.

- **דו-לשוני HE/EN** — מתג שפה (בהגדרות) שמהפך גם את כיוון המסך (RTL/LTR).
- **בורר צבע ראשי** בהגדרות — 5 גוונים (נייבי / טורקיז / ירוק-זית / תכלת / סגול),
  נשמר ב-localStorage ומשנה את צבע הכפתורים וההדגשים בכל האפליקציה.
- שכבת הנתונים (Supabase `api()` shim) לא השתנתה — רק שכבת התצוגה.
- הגרסה הישנה ("Garden") נשמרה באפליקציית ה-Node תחת `public/`.

> מקור: handoff מ-Claude Design (`AmitVet.html`). כל הטוקנים ב-`docs/styles.css`.
