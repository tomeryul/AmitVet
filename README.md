...
## עיצוב — "AmitVet Care" (clean clinical, mobile-first)

הממשק (ב-`docs/`) מעוצב בשפת **"AmitVet Care"**: מראה רפואי נקי ואמין בהשראת
אפליקציות בריאות, **mobile-first**. פלטת כחול רפואי (`#1773c4`) + טורקיז (`#0fa39a`)
על קנבס קריר; גופני **Rubik + Assistant** (שניהם תומכים עברית *ואנגלית*); אייקוני
**Lucide** ואווטארים בגרדיאנט (ללא אמוג'י). מבנה נייד: app bar שקוף, **טאב-בר תחתון**,
כפתורי **FAB**, ו-**bottom-sheets** לטפסים.

- **דו-לשוני HE/EN** — מתג שפה (בהגדרות) שמהפך גם את כיוון המסך (RTL/LTR). כל מחרוזות
  הממשק עוברות דרך שכבת i18n; נתוני המשתמש נשמרים כפי שהם.
- שכבת הנתונים (Supabase `api()` shim) לא השתנתה — רק שכבת התצוגה עוצבה מחדש.
- הגרסה הקודמת ("Garden") נשמרה באפליקציית ה-Node הישנה תחת `public/`.
